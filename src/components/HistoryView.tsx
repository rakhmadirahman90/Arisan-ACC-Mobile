        ...
        <AnimatePresence mode="wait">
          {subTab === "silsilah" && (
            ...
            >
              ...
              <div className="shrink-0 flex flex-col gap-4 md:col-span-5 md:h-full">
              ...
              <div className="flex-1 overflow-y-auto scrollbar-none pr-0.5 mt-4 md:col-span-7 md:mt-0">
                <div className="relative space-y-3 before:absolute before:left-5 before:top-2 before:bottom-2 before:w-[1px] before:bg-white/5 pb-4 md:pb-6">
          ...
          {subTab === "keuangan" && (
            ...
            >
              ...
              <div className="flex flex-col gap-4 pr-0.5 pb-4 md:col-span-7 md:pb-6 md:h-full scrollbar-none">
                
                {/* SEARCH FILTER FOR TRANSPARENCY CHECKLIST - Section 1 */}
                <div className={`flex flex-col min-h-0 flex-1 bg-white/[0.01] border border-white/5 p-4 rounded-xl hover-glow-${activeLivery.id || "blue"}`}>
                  ...
                  {/* UNPAID/PAID LISTS */}
                  <div className="flex-1 min-h-0 overflow-y-auto scrollbar-none space-y-3">
                          ...
                          {filteredUnpaid.length > 0 ? (
                            filteredUnpaid.map((m, index) => (
                              <div key={m.id} className="bg-[#ffe4e6]/5 border border-red-500/10 rounded-xl p-2 flex justify-between items-center gap-2">
                                ...
                                <span className="text-[9px] text-[#f43f5e] font-mono font-bold shrink-0">{formatRupiah(activeConfig.contributionAmount)}</span>
                              </div>
                            ))
                          ) : (
                            <p className="text-[8.5px] font-mono text-zinc-650 text-center py-1 italic">Semua lunas!</p>
                          )}
                      </div>
                    </div>
                ...
                {/* HISTORICAL LEDGERS - Section 2 */}
                <div className="flex flex-col flex-1 min-h-0 space-y-2">
                  <h3 className="text-[9.5px] font-black uppercase tracking-wider text-zinc-400 font-mono flex items-center gap-1.5 px-1 shrink-0">
                    📊 Laporan Per Putaran & Sisa Arisan
                  </h3>
                  
                  {/* Search bar for Historical Ledgers */}
                  <div className="relative shrink-0 mb-2 px-1">
                    <Search className="absolute left-3 top-2.5 w-3.5 h-3.5 text-zinc-500" />
                    <input
                      type="text"
                      placeholder="Cari putaran atau pemenang..."
                      value={historySearchQuery}
                      onChange={(e) => setHistorySearchQuery(e.target.value)}
                      className="w-full bg-white/5 text-[10.5px] border border-white/10 rounded-xl pl-9 pr-4 py-2 text-[#f4f4f5] focus:outline-none focus:border-emerald-500/50 placeholder:text-zinc-650"
                    />
                  ...
                  <div className="flex-1 min-h-0 overflow-y-auto scrollbar-none space-y-2">
                    ...
                    {filteredHistory.map((h, index) => {
                      const rArisan = h.prizeAmount;
                      const rConsum = h.participantsCount * consumptionShare;
                      return (
                        <div 
                          key={h.id}
          ...
          {subTab === "ai-advisor" && (
            ...
            >
              ...
              <div className="md:col-span-5 flex flex-col space-y-4 overflow-y-auto scrollbar-none pr-0.5 shrink-0 md:h-full max-h-[50vh] md:max-h-full pb-3 md:pb-6">
                {/* BRAND HEADER CARD */}
                ...
                <div className="bg-white/5 border border-white/10 rounded-2xl p-3.5 space-y-3 shrink-0">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="grid grid-cols-2 gap-1.5">
                      <button
                        ...
                        className={`py-1.5 px-2 rounded-lg text-[9px] font-bold font-mono transition flex items-center gap-1 border ${aiCategory === "umum" ? `${activeLivery.bgPill} ${activeLivery.textPill} ${activeLivery.borderAccent}` : "bg-black/40 border-white/5 text-zinc-400 hover:bg-black/60"}`}
                      >
                      ...
                      <button
                        ...
                        className={`py-1.5 px-2 rounded-lg text-[9px] font-bold font-mono transition flex items-center gap-1 border ${aiCategory === "otomotif" ? `${activeLivery.bgPill} ${activeLivery.textPill} ${activeLivery.borderAccent}` : "bg-black/40 border-white/5 text-zinc-400 hover:bg-black/60"}`}
                      >
                      ...
                      <button
                        ...
                        className={`py-1.5 px-2 rounded-lg text-[9px] font-bold font-mono transition flex items-center gap-1 border ${aiCategory === "traktir" ? `${activeLivery.bgPill} ${activeLivery.textPill} ${activeLivery.borderAccent}` : "bg-black/40 border-white/5 text-zinc-400 hover:bg-black/60"}`}
                      >
                      ...
                      <button
                        ...
                        className={`py-1.5 px-2 rounded-lg text-[9px] font-bold font-mono transition flex items-center gap-1 border ${aiCategory === "custom" ? `${activeLivery.bgPill} ${activeLivery.textPill} ${activeLivery.borderAccent}` : "bg-black/40 border-white/5 text-zinc-400 hover:bg-black/60"}`}
                      >
                  ...
                  <button
                    ...
                    className={`w-full py-2 bg-gradient-to-r ${activeLivery.btnGrad} text-white font-mono font-black text-[10px] rounded-xl flex items-center justify-center gap-2 relative overflow-hidden transition shadow-lg cursor-pointer ${advisorLoading ? "opacity-75 cursor-not-allowed" : "hover:scale-[1.02] active:scale-[0.98]"}`}
                  >
                    {advisorLoading ? (
                      <>
              ...
              {/* Right Column result panel & help details */}
              <div className="flex-1 md:col-span-7 flex flex-col space-y-4 overflow-y-auto scrollbar-none pr-0.5 min-h-0 md:max-h-full pb-4 md:pb-6">
